...
class App extends Component {
  ...
  render() {
    ...
    let userAvatar = ''
    if(users && authid){
       userName = users[authid].name === undefined ? "" : users[authid].name
       userAvatar = users[authid].avatarURL === undefined ? '' :  avatars[users[authid].avatarURL].image
    }
    return (
      <Router>
        <Fragment>
            ...
            <AppBar position="static" className="barcont">
              <Toolbar className='appbar' >
                ...
                <div className='authuser'>
                  ...
                  <Button color="inherit" onClick={() => this.handleLogoff(userName)} >{userName ? 'sign off' : ''}</Button>
                </div>
              </Toolbar>
            </AppBar>
            {this.props.loading
