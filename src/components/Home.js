    ...
    return(
      <div>
        <ButtonGroup variant="text" color="primary" aria-label="outlined primary button group">
          ...
          <Button onClick={() => this.handleUnAnswered()}>Unanswered <AssignmentLateIcon style={{ color: 'f90', fontSize: 30}}  /></Button>
        </ButtonGroup>
        <div className='homegrid'>
        {this.state.questionid.map((id) =>(
          <form className={answered.join().indexOf(id) !== -1 ? 'homepoll' : 'homepollU'} key={id} >
            <div className='addBarPoll' >
            {answered.join().indexOf(id) !== -1
              ? <div></div>
              : <AssignmentLateIcon className='appIcon' style={{ color: 'f90', fontSize: 30}}  />}
              <h3 className='center' >{users[questions[id].author].name} asks</h3>
            </div>
            <div className='pollheader'>
              ...
              <div className='homeQuestion'>
                ...
                <span className='hometxt'>{questions[id].optionOne.text.substr(0, 14)} ...</span>
              <Button
                className="viewbtn"
                variant="contained"
                color="primary"
            ...
            <div className='pollfooter' >
              <span>Created on {this.getCreatedDate(questions[id].timestamp)}</span>
            </div>
         </form>
        ))}
        </div>
...
function mapStateToProps({questions, authid, users}){
  ...
  const answered = (users && authid)
      ? Object.keys(users[authid].answers)
      : undefined;

  const unAnswered = (users && authid && questions)
